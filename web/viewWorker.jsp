...
<html>
    ...
    <body id="home" style='background:white;overflow-y: scroll'>
        ...
        <div class="row-offcanvas row-offcanvas-left" style="padding:1% 0% 0%;">
                                    ...
                                    <div class="row">
                                        <form method="POST" action='changeToSelected'>
                                            <div class="form-group">
                                                ...
                                                <button type='submit' onclick="" class="btn btn-blue  col-md-1">Submit</button>
                                                <%
                                                    ArrayList<Problem> referredProbList = CaseManagementDAO.retrieveUnassignedCases();
                                                    boolean is_referred = false;
                                                    for (Problem p : referredProbList) {
                                                        int tempProbKey = p.getProbKey();

                                                        if (latestProblem.getProbKey() == tempProbKey) {
                                                    ...
                                                    if(is_referred){
                                                %>
                                                <button type='button' class="btn btn-disabled col-md-offset-1 col-md-1" id="refer_case_btn">Case Referred</button>                                       
                                                <%
                                                    }else{
                                                %>
                                                <button type='button' onclick="referCase();" data-title="Case Referral" class="btn btn-blue col-md-offset-1 col-md-1" id="refer_case_btn">Refer Case</button>                                       
                                                <%        
                                                    }
                                                %>
            ...
            $(document).on("click", ".pop_up_open", function() {
                ...
                if (div_action_val === 'viewedit' || div_class === 'benefection') {
                    $("#pop_up_content").load('include/' + url + '?workerFin=<%=workerFin%>&' + div_id + '=' + div_value
                            + "&jobkey=<%=latestJob.getJobKey()%>" + "&probkey=<%=latestProblem.getProbKey()%>&" + div_id + "=" + div_value + "&" + "&action=" + div_action_val).dialog({modal: true,  minWidth: $(window).width() - 750, resizable: false, title: div_title, draggable: false, position: ['center',$(window).height() * 0.18],close: function() {
                            $(this).dialog('destroy');
                            $('#pop_up_content').empty();
                        }
                    });
                    ...
                    $("#pop_up_content").load('include/addPopUp.jsp?workerFin=<%=workerFin%>&complement=' + div_id
                        ...
                        minWidth: $(window).width() - 750, title: div_title, resizable: false, draggable: false, position: ['center',$(window).height() * 0.18], close: function() {
                            $(this).dialog('destroy');
                            $('#pop_up_content').empty();
                        }});
                }
            ...
            function referCase() {
                $("#pop_up_content").load('include/referCase.jsp?workerFin=<%=workerFin%>&jobkey=<%=latestJob.getJobKey()%>&probkey=<%=latestProblem.getProbKey()%>&user=<%= userLogin.getNricNumber()%>').dialog({modal: true, minWidth: $(window).width() - 750, title: "Case Referral", resizable: false, draggable: false, position: ['center',$(window).height() * 0.18],close: function() {
                        $(this).dialog('destroy');
                        $('#pop_up_content').empty();
                    }});
            }
